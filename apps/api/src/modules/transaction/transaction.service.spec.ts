...
describe('TransactionService', () => {
  ...
  describe('create', () => {
    it('should create a transaction and update account balance (Income)', async () => {
      ...
      mockTransactionClient.account.findUnique.mockResolvedValue(mockAccount);
  ...
  describe('findAll', () => {
    it('should return an array of transactions', async () => {
      const result = [{ id: '1' }];
      (prisma.transaction.findMany as jest.Mock).mockResolvedValue(result);
  ...
  describe('findOne', () => {
    it('should return a single transaction', async () => {
      const result = { id: '1' };
      (prisma.transaction.findUnique as jest.Mock).mockResolvedValue(result);
  ...
  describe('update', () => {
    it('should update a transaction and recalculate balance (Amount change)', async () => {
      ...
      const id = 'tx-1';
      const dto = { amount: 200, accountId: 'acc-1', type: 'INCOME' } as any;
      ...
      expect(mockTransactionClient.account.update).toHaveBeenNthCalledWith(1, {
        where: { id: 'acc-1' },
        data: { balance: 50 },
      });

      // Verify Apply New
      expect(mockTransactionClient.account.update).toHaveBeenNthCalledWith(2, {
        where: { id: 'acc-1' },
        data: { balance: 250 },
      });
    });
  ...
  describe('remove', () => {
    it('should delete transaction and revert balance', async () => {
      ...
      expect(mockTransactionClient.transaction.delete).toHaveBeenCalledWith({
        where: { id },
      });
    });
  });
